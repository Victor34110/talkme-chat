...
export default function AddMember({params,
  ...
  const fetchUsers = async () => {
    try {
      const [serverRes, channelRes] = await Promise.all([
        fetch(`${ENV.API_HTTP}/servers/${serverId}/members`, {
          credentials: "include",
        }),
        fetch(`${ENV.API_HTTP}/channels/${channelId}/members`, {
          credentials: "include",
        }),
      ]);

      if (!serverRes.ok || !channelRes.ok) {
      ...
      const channelUsers: User[] =
        channelData.users ?? (Array.isArray(channelData) ? channelData : []);

      const channelUserIds = new Set(channelUsers.map(user => user.id));

      setUsers(serverUsers.filter(user => !channelUserIds.has(user.id)));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const url = `${ENV.API_HTTP}/channels/${channelId}/join`;
